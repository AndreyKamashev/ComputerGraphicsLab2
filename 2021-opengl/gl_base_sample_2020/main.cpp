...
int main(int argc, char** argv)
{
    ...
    GLuint vaoLeg;
    ////////////////////////////
    GLsizei sphereIndices = CreateSphere(1.0f, 100, vaoSphere);
    GLsizei Plane = CreatePlane(vaoPlane);
    GLsizei Cuboid = CreateCuboid(0.5f, 0.5f, 0.5f, vaoCuboid);
    GLsizei Conus = CreateConus(1.0f, 4.0f, 100, vaoConus);
    GLsizei Cylinder = CreateCylinder(1.0f, 4.0f, 100, vaoCylinder);
    GLsizei Forest = CreateConus(1.0f, 4.0f, 150, vaoForest);
    GLsizei Hat = CreateConus(1.0f, 4.0f, 150, vaoHat);
    GLsizei Leg = CreateCylinder(1.0f, 4.0f, 20, vaoLeg);
    ////////////////////////////////////////////////////////////////////////////////
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindVertexArray(vaoForest);
        {
            for (int i = 1; i < 500; i++)
            {
                float4x4 model1 = transpose(mul(translate4x4(float3(sin(20 * i) * 19.0f, -5.99f, cos(i) * 19.0f)), scale4x4(float3(0.6f, (0.8 + sin(i / 2) / 3 * pow((-1), i)) * 1.0f, 0.4f))));
        ...
        glBindVertexArray(vaoSphere);
        {
           ...
           /*modelBee = transpose(mul(translate4x4(float3(1.0f+i+sin(timer*50), 0.0f+i+cos(timer*5), 0.0f)), scale4x4(float3(1.0f, 1.0f, 1.0f))));
            ...
            modelHat = transpose(mul(translate4x4(float3(1.0f + 15 * sin(timer * 5), 6.0f , 1.0f + 15 * cos(timer * 5))), scale4x4(float3(1.0f, 1.0f, 1.0f))));
            //modelHat = transpose(mul(translate4x4(float3(1.0f + i + 15 * sin(timer * 5), 1.0f, 1.0f + i + 15 * cos(timer * 5))), scale4x4(float3(1.0f, 1.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, sphereIndices, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            modelHat = transpose(mul(translate4x4(float3(1.0f + 10 * sin(timer * 10), 6.0f, 1.0f + 10 * cos(timer * 10))), scale4x4(float3(0.6f, 0.6f, 0.6f))));
            //modelHat = transpose(mul(translate4x4(float3(1.0f + i + 15 * sin(timer * 5), 1.0f, 1.0f + i + 15 * cos(timer * 5))), scale4x4(float3(1.0f, 1.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, sphereIndices, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

            modelHat = transpose(mul(translate4x4(float3(1.0f + 21 * sin(timer * 4), 6.0f, 1.0f + 21 * cos(timer * 4))), scale4x4(float3(1.6f, 1.6f, 1.6f))));
            //modelHat = transpose(mul(translate4x4(float3(1.0f + i + 15 * sin(timer * 5), 1.0f, 1.0f + i + 15 * cos(timer * 5))), scale4x4(float3(1.0f, 1.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, sphereIndices, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
        ...
        glBindVertexArray(vaoCylinder);
        {
            ...
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(2.0f, -1.0f + 10 * timer * timer / 2, 2.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            //modelHat = transpose(mul(translate4x4(float3(1.0f + i + 15 * sin(timer * 5), 1.0f, 1.0f + i + 15 * cos(timer * 5))), scale4x4(float3(1.0f, 1.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
        ...
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(2.0f, 7.0f + 10 * timer * timer / 2, 2.0f)), scale4x4(float3(1.2f, 1.5f, 1.2f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
        ...
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(1.0f, -2.0f + 10 * timer * timer / 2, 1.0f)), scale4x4(float3(4.0f, 10.0f, 4.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
        ...
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(1.0f, -6.0f + 10 * timer * timer / 2, 1.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Cylinder, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
        glBindVertexArray(vaoCylinder);
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(3.0f, -6.0f + 10 * timer * timer / 2, 3.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Cylinder, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
        glBindVertexArray(vaoCylinder);
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(3.0f, -6.0f + 10 * timer * timer / 2, 1.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Cylinder, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
        glBindVertexArray(vaoCylinder);
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(1.0f, -6.0f + 10 * timer * timer / 2, 3.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Cylinder, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
        glBindVertexArray(vaoConus);
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(1.0f, -9.0f + 10 * timer * timer / 2, 1.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Cylinder, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
        glBindVertexArray(vaoConus);
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(3.0f, -9.0f + 10 * timer * timer / 2, 3.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Cylinder, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
        glBindVertexArray(vaoConus);
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(3.0f, -9.0f + 10 * timer * timer / 2, 1.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Cylinder, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
        glBindVertexArray(vaoConus);
        {
            float4x4 modelHat;
            modelHat = transpose(mul(translate4x4(float3(1.0f, -9.0f + 10 * timer * timer / 2, 3.0f)), scale4x4(float3(1.0f, 2.0f, 1.0f))));
            lambert.SetUniform("model", modelHat); GL_CHECK_ERRORS;
            glDrawElements(GL_TRIANGLE_STRIP, Cylinder, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
        }
